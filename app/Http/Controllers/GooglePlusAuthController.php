...
class GooglePlusAuthController extends Controller {
    ...
    */
    public function connect()
    {
        ...
        $authorize_url = GooglePlusSM::getAuthorizeURL($url);
    ...
    */
    ...
    {
        $access_token = GooglePlusSM::mirror(Request::all());
        $access_token = json_decode(json_encode($access_token), true);
        
        $params['user_id'] = $access_token['id'];
        $params['screen_name'] = $access_token['name'];
        $params['image'] = $access_token['picture'];
        $params['social_media'] = 'googleplus';
        $params['active'] = true;
        $params['consumer_key'] = Config::get('oauth-5-laravel.consumers')['Google']['client_id'];
        $params['consumer_secret'] = Config::get('oauth-5-laravel.consumers')['Google']['client_secret'];
